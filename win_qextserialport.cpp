...
bool Win_QextSerialPort::open(OpenMode mode) {
    ...
    if (!isOpen()) {
        ...
        if (Win_Handle!=INVALID_HANDLE_VALUE) {
			...
			if (queryMode() == QextSerialBase::EventDriven) {
		        Win_CommTimeouts.ReadIntervalTimeout = MAXDWORD;//两字符之间最大的延时，当读取串口数据时，一旦两个字符传输的时间差超过该时间，读取函数将返回现有的数据。设置为0表示该参数不起作用。
		        Win_CommTimeouts.ReadTotalTimeoutMultiplier = 0;//读取每字符间的超时。 
		        Win_CommTimeouts.ReadTotalTimeoutConstant = 0;//一次读取串口数据的固定超时。
				Win_CommTimeouts.WriteTotalTimeoutMultiplier = 0;//写入每字符间的超时。
				Win_CommTimeouts.WriteTotalTimeoutConstant = 0;//一次写入串口数据的固定超时。
...
//@todo implement
	//SetCommMask(Win_Handle, EV_RXCHAR);//设置可接收事件   必须与WaitCommEvent配合使用

	//DWORD dwEventMask;
	//if(WaitCommEvent(Win_Handle, &dwEventMask, NULL))//接收事件
	//{
	//	return true;
	//}
	//else
	...
	if (queryMode() == QextSerialBase::EventDriven) {
		...
		Win_CommTimeouts.ReadTotalTimeoutMultiplier = 0;//读取每字符间的超时。 
		Win_CommTimeouts.ReadTotalTimeoutConstant = 0;//一次读取串口数据的固定超时。
		Win_CommTimeouts.WriteTotalTimeoutMultiplier = 50;//写入每字符间的超时。
		Win_CommTimeouts.WriteTotalTimeoutConstant = 2000;//一次写入串口数据的固定超时。
		SetCommTimeouts(Win_Handle, &Win_CommTimeouts);
		if (!SetCommMask( Win_Handle, EV_TXEMPTY | EV_RXCHAR | EV_DSR)) {
			qWarning("Failed to set Comm Mask. Error code: %ld", GetLastError());
			UNLOCK_MUTEX();
